...
#import "MPPopMenu.h"
@interface MPHomeViewController()<MPCoverDelegate>

@property (nonatomic, weak) MPTittleButton *titleButton;
@property (nonatomic, strong) MPChoiceViewController *choice;

@end
@implementation MPHomeViewController
- (MPChoiceViewController *)choice {
    if (_choice == nil) {
        _choice = [[MPChoiceViewController alloc] init];
    }
    return _choice;
}
...
//    titleButton.frame = self.navigationItem.titleView.frame;
...
- (void)titleClick:(UIButton *)button
{
    ...
    CGFloat popH = popW;
    CGFloat popY = 55;
    MPPopMenu *menu = [MPPopMenu showInRect:CGRectMake(popX, popY, popW, popH)];
...
- (void)pop
{
    MPChoiceViewController *choice = [[MPChoiceViewController alloc] init];
