...
#import "MPPhotoView.h"
#import "MJPhotoBrowser.h"
#import "MJPhoto.h"
#import "MJExtension.h"

#define MPPhotoCount 9

const CGFloat photoMargin = 10;
const CGFloat photoWH = 70;


@implementation MPPhotosView
...
{
    for (int i = 0; i < MPPhotoCount; i++) {
        MPPhotoView *imageV = [[MPPhotoView alloc] init];
        imageV.tag = i;
...
- (void)tap:(UITapGestureRecognizer *)tap
{
    ...
    for (NSDictionary *dic in _pic_urls) {
        MPPhoto *photo = [MPPhoto mj_objectWithKeyValues:dic];
...
- (void)setPic_urls:(NSArray *)pic_urls
{
    ...
    for (int i = 0; i < MPPhotoCount; i++) {
        MPPhotoView *imageV = self.subviews[i];
        
        if (i >= photosCount) { // 大于总的个数，就不需要显示
            imageV.hidden = YES;
        }else{
            MPPhoto *photo = [MPPhoto mj_objectWithKeyValues:self.pic_urls[i]];
            imageV.photo = photo;
