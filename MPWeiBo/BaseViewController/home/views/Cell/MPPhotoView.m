//
//  MPPhoto.m
//  ItcastWeibo
//
//  Created by yz on 14/11/14.
//  Copyright (c) 2014年 iThinker. All rights reserved.
//

#import "MPPhotoView.h"

#import "MPPhoto.h"
#import "UIImageView+WebCache.h"
#import "UIView+Frame.h"


@interface MPPhotoView ()

@property (nonatomic, weak) UIImageView *gifView;

@end

@implementation MPPhotoView

- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        self.contentMode = UIViewContentModeScaleAspectFill;
        self.clipsToBounds = YES;
        self.userInteractionEnabled = YES;
    }
    return self;
}

- (UIImageView *)gifView
{
    if (_gifView == nil) {
        UIImageView *gifV = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"timeline_image_gif"]];
        
        _gifView = gifV;
        
        [self addSubview:gifV];
    }
    return _gifView;
}

- (void)setPhoto:(MPPhoto *)photo
{
    _photo = photo;
    [self sd_setImageWithURL:photo.thumbnail_pic placeholderImage:[UIImage imageNamed:@"timeline_image_placeholder"]];
    if ([_photo.thumbnail_pic.absoluteString hasSuffix:@".gif"]) { // gif图片
        self.gifView.hidden = NO;
    }else{
        self.gifView.hidden = YES;
    }
}

- (void)layoutSubviews
{
    [super layoutSubviews];
    self.gifView.left = self.width - self.gifView.width;
    self.gifView.top = self.height - self.gifView.height;
    
}

@end
