...
#import "MPStatus.h"
#define MPStatusFile [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0] stringByAppendingPathComponent:@"status.sqlite"]

static FMDatabase *_db;

@implementation MPStatusCacheTool

+ (void)initialize
{
    // 创建FMDatabase
    _db = [FMDatabase databaseWithPath:MPStatusFile];
...
{
    NSString *accessToken = [MPAccountTool account].access_token;
...
+ (NSArray *)statusesWithParam:(MPStatusParam *)param
{
    ...
    while ([set next]) {
        ...
        MPStatus *s = [MPStatus objectWithKeyValues:dict];
        [arrM addObject:s];
    }
